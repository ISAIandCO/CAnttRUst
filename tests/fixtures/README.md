...
`ct-log-list.json` — фиксированный снимок политики 1.229 для криптографических и
сценарных тестов. Тестовые SCT проверяются по своим историческим ключам/статусам,
чтобы реальное отклонение или удаление лога в рабочей политике не ломало тест
«историческая подпись корректна» и не препятствовало выпуску обновления доверия.
Рабочая политика отдельно проверяется `verify:policy` при каждой сборке.
